 ...
 * @return HTTP 200 if success, 400 if fail. Response body 에는 name 과 JWT Token 이 포함됨.
 */
export const login: Handler = async (event: APIGatewayEvent, context: Context) => {
  try {
    const user: B1imeUser = JSON.parse(event.body as string);
    await B1imeUser.create(user);
